...
void ImageViewer::ViewerWidgetMouseButtonPress(ViewerWidget* w, QEvent* event)
{
	...
	if (e->button() == Qt::LeftButton && ui->toolButtonDrawLine->isChecked()) //рисуетс€ только если кнопка зажата в уи
	{
		if (w->getDrawLineActivated()) {
			QPoint start = w->getDrawLineBegin();
			QPoint end = e->pos();
			float radius = sqrt(pow(end.x() - start.x(), 2) + pow(end.y() - start.y(), 2));//радиус линии зависит от длины отрезка, 100 - это просто коэффициент дл€ удобного отображени€
